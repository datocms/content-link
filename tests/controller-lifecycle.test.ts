...
describe('Controller lifecycle', () => {
  it('should find stega elements after dispose and recreate', () => {
    ...
    const imageWrapper1 = heroImage.closest(`[${AUTOMATIC_TARGET_STAMP_ATTRIBUTE}]`) as HTMLElement;
    ...
    const imageWrapper1After = heroImage.closest(`[${AUTOMATIC_TARGET_STAMP_ATTRIBUTE}]`);
    ...
    const imageWrapper2 = heroImage.closest(`[${AUTOMATIC_TARGET_STAMP_ATTRIBUTE}]`) as HTMLElement;
  ...
  it('strips stega when stripStega option is true', () => {
    ...
    expect(textElement.hasAttribute(AUTOMATIC_TARGET_STAMP_ATTRIBUTE)).toBe(false);
  ...
  it('shows that manual stamping still works after controller recreation', () => {
    ...
    expect(manualElement.getAttribute('data-datocms-edit-url')).toBe('manual-1#fieldPath=title');
