...
const controller = createController({
  ...
  // Optional: disable Content Link console warnings (default: true)
  warnings: false,
...
- `warnings?: boolean`: Whether to emit Content Link diagnostic warnings to the console (default: `true`). Set to `false` only when you have reviewed and accepted the warning source.
...
If the warning is expected and you have accepted the ambiguous target, you can silence Content Link warnings with `createController({ warnings: false })`. This only suppresses the console warning; it does not change the underlying stamping behavior.
...
- **Known warning noise**: If you have reviewed the warning source and want to keep the current markup, pass `warnings: false` to `createController()`. This silences Content Link warnings without changing which elements are stamped.
